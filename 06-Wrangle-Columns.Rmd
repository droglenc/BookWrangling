...
because `%>%` will "pipe" `old_df` into the first argument of `verb()`{.inline}.
...
## Renaming Variables
Variables may be given new names with `rename()`{.inline}. Each directive argument in `rename()`{.inline} has the form `newname=oldname` where `newname` will be the new name for the `oldname` variable in the data frame. Below are examples of renaming variables in `bears`.
