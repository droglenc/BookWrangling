...
Numeric day of the week is returned by `wday()`{.inline}.^[By default the week day starts on "Sunday."] The abbreviated name of the week is returned when `label=TRUE` is included and the full names is returned when `abbr=FALSE` is also included. The data type will also be "ord" when labels are returned.
...
Use `%within%` to determine if a date is within an interval and `int_overlaps()`{.inline} to determine if two intervals overlap.
...
The amount of time within an interval is found with `time_length()`{.inline} with units of measurement supplied to `unit=`. When `units=` `"months"` or `"years"` then `time_length()`{.inline} will take into account that not all months and years have the same number of days.^[As long as `time_length()`{.inline} is given an interval class type.]
...
* Copied URL from the ensuing page into `read_tsv()`{.inline} below. [*Note that the top of the file has many lines of comments each preceded by a '#', which precipates the use of `comment=` below.*]
