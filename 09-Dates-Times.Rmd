...
The month-year variable should be converted to a factor for summarization but the order needs to be controlled. A quick look at the unique values in the `monyr` variable show that they are already in order so the results of this function can be used in `levels=`{.inline} within `factor()`{.inline} to put the month-years in order.
